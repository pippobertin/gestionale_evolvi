-- Fix funzione autocompilazione con campo corretto url_file

-- Elimina funzione attuale
DROP FUNCTION IF EXISTS auto_compila_allegato_progetto(UUID, UUID) CASCADE;

-- Crea funzione corretta usando url_file invece di file_path
...
DECLARE
    ...
    result JSON;
BEGIN
    -- Ottieni documento progetto + documento bando originale con campo CORRETTO
    SELECT
        dp.*,
        bd.url_file as template_path,          -- ✅ CAMPO CORRETTO
        bd.nome_file as template_filename,
        'bandi-documenti' as template_bucket   -- Bucket dei documenti bandi
    INTO doc_record
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ FUNZIONE CORRETTA CON url_file!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ Template path: bd.url_file (CAMPO CORRETTO)';
    RAISE NOTICE '✅ Template bucket: bandi-documenti';
    RAISE NOTICE '✅ Upload bucket: progetti-documenti';
    RAISE NOTICE '🎯 RIPROVA AUTOCOMPILAZIONE ORA!';
    RAISE NOTICE '========================================';
END $$;