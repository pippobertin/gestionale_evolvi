-- Fix finale vista progetti con nomi colonne corretti verificati

-- Elimina la vista esistente se presente
DROP VIEW IF EXISTS scadenze_bandi_progetti_view CASCADE;

-- Crea la vista progetti con nomi colonne corretti
...
SELECT
    ...
    -- Info cliente collegato (nomi corretti verificati)
    ...
    -- Conteggio documenti (usa la nuova tabella documenti progetti)
    COALESCE(
...
BEGIN
    ...
    IF view_exists THEN
        -- Testa la vista con una query
        SELECT COUNT(*) INTO sample_count FROM scadenze_bandi_progetti_view;

        -- Prendi un record di esempio per verificare le colonne
        SELECT
        ...
        RAISE NOTICE '✅ Colonne corrette: denominazione -> cliente_denominazione';
        ...
        RAISE NOTICE '=========================================';
        RAISE NOTICE 'PROSSIMO: Ricarica pagina progetti e testa tab Documenti';
        RAISE NOTICE '=========================================';
    ELSE
