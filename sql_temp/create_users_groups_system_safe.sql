-- Sistema di gestione utenti e gruppi per responsabili scadenze (versione safe)
...
BEGIN
    -- Trigger per utenti
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_scadenze_bandi_utenti_updated_at') THEN
...
-- 8. Inserisci alcuni dati di esempio (solo se non esistono)
