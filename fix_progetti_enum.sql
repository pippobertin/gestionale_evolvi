-- Fix per conflitto ENUM stati progetto
-- Questo risolve il problema con gli stati progetto esistenti

-- 1. Prima controlla se esiste già un ENUM per stati progetto e lo elimina
DO $$
BEGIN
    -- Elimina la colonna stato se esiste per poter ricreare l'ENUM
    ALTER TABLE scadenze_bandi_progetti DROP COLUMN IF EXISTS stato;

    -- Elimina i tipi ENUM esistenti se presenti
...
-- 2. Crea il nuovo ENUM con i valori corretti
...
-- 4. Aggiorna tabella progetti con tutti i campi necessari
...
ADD COLUMN IF NOT EXISTS modalita_calcolo_scadenza_rendicontazione TEXT DEFAULT 'decreto_concessione',
...
-- 6. Vista progetti con calcoli automatici
...
-- 7. Funzione per creare scadenze automatiche
...
BEGIN
    ...
    -- Crea scadenza accettazione esiti
    ...
    -- Crea scadenza richiesta anticipo
