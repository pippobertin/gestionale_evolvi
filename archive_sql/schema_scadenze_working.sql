-- Schema completo per il sistema di gestione scadenze (WORKING VERSION)
...
-- 5. Tabella principale scadenze (PRIMA senza foreign key)
CREATE TABLE IF NOT EXISTS scadenze_bandi_scadenze (
    ...
    -- Relazioni (UUID senza foreign key per ora)
    cliente_id UUID,
...
-- 6. ORA aggiungiamo la foreign key SOLO se la tabella clienti esiste E ha la struttura corretta
DO $$
BEGIN
    -- Controlla se la tabella clienti esiste E ha la colonna id
    IF EXISTS (
        SELECT 1
        ...
        RAISE NOTICE 'Foreign key aggiunta con successo';
    ELSE
        RAISE NOTICE 'Tabella clienti non trovata o senza colonna id, foreign key non aggiunta';
    END IF;
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Errore nell''aggiunta della foreign key: %', SQLERRM;
END $$;
...
-- 10. Vista per scadenze con calcoli automatici E info clienti
...
SELECT
    ...
    CASE
        ...
        ELSE 'NORMALE'
    END as urgenza,
    -- Info cliente collegato (se esiste la tabella e il collegamento)
...
-- 14. Collega le scadenze ai clienti esistenti (se presenti)
DO $$
DECLARE
    primo_cliente_id UUID;
BEGIN
    -- Cerca il primo cliente disponibile
    SELECT id INTO primo_cliente_id FROM scadenze_bandi_clienti LIMIT 1;

    IF primo_cliente_id IS NOT NULL THEN
        -- Collega tutte le scadenze senza cliente al primo cliente
        UPDATE scadenze_bandi_scadenze
        SET cliente_id = primo_cliente_id
        WHERE cliente_id IS NULL;

        RAISE NOTICE 'Scadenze collegate al cliente %', primo_cliente_id;
    ELSE
        RAISE NOTICE 'Nessun cliente trovato per collegare le scadenze';
    END IF;
EXCEPTION
    WHEN others THEN
        RAISE NOTICE 'Errore nel collegamento clienti: %', SQLERRM;
END $$;
