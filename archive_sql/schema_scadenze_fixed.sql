-- Schema completo per il sistema di gestione scadenze (CORRETTO)
...
-- 4. Tabella principale scadenze (SENZA foreign key per ora)
CREATE TABLE IF NOT EXISTS scadenze_bandi_scadenze (
    ...
    -- Relazioni (per ora solo ID senza foreign key)
    cliente_id UUID, -- Collegamento opzionale ai clienti
    cliente_nome VARCHAR(255), -- Nome cliente per display
...
-- 11. Dati di esempio per testing
