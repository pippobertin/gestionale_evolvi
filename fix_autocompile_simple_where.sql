-- Fix funzione autocompilazione semplificando WHERE per evitare ambiguità

-- Elimina funzione attuale
DROP FUNCTION IF EXISTS auto_compila_allegato_progetto(UUID, UUID) CASCADE;

-- Crea funzione corretta con WHERE semplificato
...
DECLARE
    ...
    result JSON;
BEGIN
    -- Ottieni documento progetto + documento bando originale con WHERE semplificato
    SELECT
    ...
    WHERE dp.id = documento_id;  -- ✅ Solo documento_id, no ambiguità
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ FUNZIONE SENZA AMBIGUITÀ!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ WHERE semplificato: solo documento_id';
    RAISE NOTICE '✅ Verifica progetto_id separata';
    RAISE NOTICE '✅ Nessuna ambiguità di colonne';
